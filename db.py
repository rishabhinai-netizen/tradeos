...
def _safe(fn):
    """Decorator: return empty DataFrame/None on any DB error instead of crashing."""
    import functools
    @functools.wraps(fn)
    def wrapper(*args, **kwargs):
        try:
            ...
            # Surface to Streamlit only in debug mode
            pass
        # Return sensible default
        import inspect
        sig = inspect.signature(fn)
        return pd.DataFrame() if "df" in fn.__name__ or fn.__name__.startswith("get_") else None
    return wrapper
